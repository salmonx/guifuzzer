...
pre_fuzz_handler(void *wrapcxt, INOUT void **user_data)
{
    ...
    if(!options.debug_mode) {
		...
		dr_fprintf(winafl_data.log, "In pre_fuzz_handler fuzz_target:%x\n", fuzz_target);
    }
...
post_fuzz_handler(void *wrapcxt, void *user_data)
{
    ...
    if(!options.debug_mode) {
		...
		dr_fprintf(winafl_data.log, "In post_fuzz_handler fuzz_target:%x\n", mc);
    }
...
static void
mfc_cfile_open_interceptor(void *wrapcxt, INOUT void **user_data)
{
	wchar_t *filenamew = (wchar_t *)drwrap_get_arg(wrapcxt, 0);
    if (options.debug_mode)
        dr_fprintf(winafl_data.log, "In Cfile::open %s\n", filenamew);
}
...
static void
skip_domodal_interceptor(void *wrapcxt, INOUT void **user_data)
{
...
static void
skip_getpathname_interceptor(void *wrapcxt, INOUT void **user_data)
{
	...
	if (options.debug_mode){
		...
		//lpszFilePath = (wchar_t *)(LPCTSTR)newfilepath;
		//lpszFilePath = (const wchar_t *)newfilepath;

		//*lpszFilePath = (int)lpszFilePath_obj + 64;
		//lpszFilePath = lpszFilePath_obj + 64;

		//wcscpy(lpszFilePath, (wchar_t *)("c:\\out\\s.cur_input"));
		//wcscpy(lpszFilePath, _T("c:\\out\\s.cur_input"));
		wcscpy(lpszFilePath , L"c:\\out\\.cur_input");

		/*
		lpszFilePath[-4] = 1;
		lpszFilePath[-8] = 0x17;
		lpszFilePath[-12] = 0x11;

		*(lpszFilePath-16) = 0x7889c560;
*/
		...
		//lpszFilePath = L"Hello World";

		//int_FilePath = (int *)drwrap_get_arg(wrapcxt, 0);
		p_FilePath = (int *)drwrap_get_arg(wrapcxt, 0);

		dr_fprintf(winafl_data.log, "in CFileDialog, before stack modify: 0x%x &0x%x, 0x%x *%s\n", p_FilePath, &p_FilePath, *p_FilePath, *p_FilePath);

		*p_FilePath = (int)lpszFilePath;

		dr_fprintf(winafl_data.log,  "newfilepath -4 : maybe ref count == 1 :%x %x\n", lpszFilePath - 4, *(lpszFilePath - 4));
		dr_fprintf(winafl_data.log,  "newfilepath -4 : maybe length == 17 :%x %x\n", *(lpszFilePath - 12), *(lpszFilePath - 8));
		dr_fprintf(winafl_data.log,  "newfilepath -4 : unkonwn header :%x\n", *(lpszFilePath - 16));
		dr_fprintf(winafl_data.log,  "newfilepath heap ADDR :%x\n",&lpszFilePath);
		dr_fprintf(winafl_data.log, "in CFileDialog, after stack modify: 0x%x &, 0x%x *%ls\n", p_FilePath,  *p_FilePath, *p_FilePath);
		//*newfilepath = fp;

		//*p_FilePath = (int)lpszFilePath;

		//p_FilePath = newfilepath;
		//*p_FilePath = _T("c:\\out\\.cur_input");

		//*int_FilePath = (int) &lpszFilePath;
		//*ptr = lpszFilePath;s
		...
		dr_fprintf(winafl_data.log, "in CFileDialog, filepatharg : 0x%x *: %x string: %s  new file path: 0x%x string: %s\n", p_FilePath, *p_FilePath,*p_FilePath, (int)drwrap_get_arg(wrapcxt, 0), *(int *)drwrap_get_arg(wrapcxt, 0));



		//MessageBox(NULL, ("hello dll"), lpszFilePath, 0);

		// mov eax , ptr->ptr->filepath
		//
...
static void
skip_destructor_cstringt_interceptor(void *wrapcxt, INOUT void **user_data){
	...
	if (options.debug_mode){
		...
		dr_fprintf(winafl_data.log, "in ~CStringT, skip it\n");
...
event_module_load(void *drcontext, const module_data_t *info, bool loaded)
{
    ...
    if(options.debug_mode)
        dr_fprintf(winafl_data.log, "Module loaded, name: %s\n", module_name);
    ...
    if(options.fuzz_module[0]) {
        ...
        if(options.debug_mode && (strcmp(module_name, "KERNEL32.dll") == 0)) {
			...
			dr_fprintf(winafl_data.log, "wrap, CreateFileW: %x\n", to_wrap);
			...
			dr_fprintf(winafl_data.log, "wrap, CreateFileA: %x\n", to_wrap);
        }


		if (options.debug_mode && (strcmp(module_name, "mfc100u.dll") == 0)) {
            //to_wrap = (app_pc)dr_get_proc_address(info->handle, "?Open@CFile@@UAEHPB_WIPAVCFileException@@@Z");


			//.text:0040502D FF 15 D8 A8 40 00       call    ds:__imp_?DoModal@CFileDialog@@UAEHXZ ; CFileDialog::DoModal(void)

			//skip domodal()

			// info->start  0x54cd0000
			// od           0x54cd0000
			// od  domodal  0x54eaf09a
			// 0x54eaf09a - 0x54cd0000 + 0x54cd0000

			to_wrap = (app_pc)0x0040a8d8; // 

			// 
			to_wrap = (app_pc)(0x787cf09a - (int)info->start + 0x785f0000);

			//drsym_enumerate_symbols("mfc100u.dll", NULL, NULL, 0);
			
			dr_fprintf(winafl_data.log, "mfc100u.dll CFileDialog::doModal: %x module address %x %d\n", to_wrap, info->start, info->start);
			drwrap_wrap(to_wrap, skip_domodal_interceptor, NULL);
			...
			dr_fprintf(winafl_data.log, "mfc100u.dll test size of a & b: %d %d\n", sizeof(*a), sizeof(*b));

			//SendMessage(NULL, WM_SETTEXT, 0, (LPARAM)"C:\\Documents and Settings\\Blah\\Desktop\\myText.txt");

		

			// skip getpathname;
			// ds:[0106A8DC]=54EAF2E0 (mfc100u.#5652) notepad.cpp:249.  CString m_FilePath = fileDlg.GetPathName();

			to_wrap = (app_pc)(0x787CF2E0 - (int)info->start + 0x785f0000);



			drwrap_wrap(to_wrap, skip_getpathname_interceptor, NULL);


			//00404EFB  |.  FF15 CCA24000         call dword ptr ds:[<&mfc100u.#902>]                             ;  mfc100u.#14057
			to_wrap = (app_pc) 0x00404EFB;
			to_wrap = (app_pc) 0x78650BEE;
			drwrap_wrap(to_wrap, skip_destructor_cstringt_interceptor, NULL);



			
			dr_fprintf(winafl_data.log, "mfc100u.dll CFileDialog::GetPathName: %x module address %x %d\n", to_wrap, info->start, info->start);

        }

		if (options.debug_mode && (strcmp(module_name, "USER32.dll") == 0)) {
			to_wrap = (app_pc)dr_get_proc_address(info->handle, "MessageBoxW");
			//result = drwrap_wrap(to_wrap, messagebox_interceptor, NULL);
			dr_fprintf(winafl_data.log, "user32.dll MessageBoxW: %x\n", to_wrap);
						
		}


		//QFile::QFile(QString const &)	Qt5Core
		if (options.debug_mode && (strcmp(module_name, "Qt5Core.dll") == 0)) {
