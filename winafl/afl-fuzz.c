...
{
	DWORD num_written;
	//ACTF("afl-fuzz write to pipe '%c'", cmd);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (result == 0) 
  {
	  ...
	  ACTF("ReadCommandFromPipe result 0 FAULT_TMOUT\n");
...
static void write_to_testcase(void* mem, u32 len) {
  ...
  if (out_file) {
	...
	fd = open(out_file, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0777);
  ...
  /*
  // split mem and store it
  // TODO: need a input info table
  u32 table[3] = {1, 1, 1};
  char* new_buf;
  u32 count = sizeof(table);

  new_buf = (char *)ck_alloc_nozero((count + 1) * 8 + len);
  sprintf(new_buf, "%08u%08u%08u", count, table[0], table[1], table[2]);
  memcpy(new_buf + 8 * count, mem, count);
  */
...
static void perform_dry_run(char** argv) {
  ...
  if (cal_failures) {
	  if (cal_failures == queued_paths)
		  WARNF("All test cases time out%s, giving up!",
...
static void maybe_delete_out_dir(void) {
  ...
  fn = alloc_printf("%s\\afl.input", out_dir);
...
static u8 fuzz_one(char** argv) {
  ...
  if (queue_cur->cal_failed) {
    ...
    if (stop_soon || res != crash_mode) {
		//ACTF("goto abandon_entry queue_cur->cal_failed:%d", queue_cur->cal_failed);
		cur_skipped_paths++;
		goto abandon_entry;
    }
...
static void sync_fuzzers(char** argv) {
  ...
  ACTF("sync_fuzzers find_pattern %s", sync_dir);
  ...
  do {
	...
	if(h2 == INVALID_HANDLE_VALUE) {
		  ACTF("sync_fuzzers  INVALID_HANDLE_VALUE");
    ...
    do {
      ...
      if (fd < 0) {
		  ACTF("sync_fuzzers  open %s failed", path);
         ck_free(path);
         continue;
      }
...
static void setup_stdio_file(void) {
  ...
  u8* fn = alloc_printf("%s\\afl.input", out_dir);

  unlink(fn); /* Ignore errors */

  //out_fd = open(fn, O_RDWR | O_BINARY | O_CREAT | O_EXCL, 0600);
  //out_fd = open(fn, O_RDWR | O_BINARY | O_CREAT | O_EXCL, 0777);
  out_fd = open(fn, O_RDWR | O_BINARY | O_CREAT | O_EXCL, _S_IREAD | _S_IWRITE);
...
static void detect_file_args(char** argv) {
  ...
  while (argv[i]) {
    ...
    if (aa_loc) {
      ...
      if (!out_file)
        out_file = alloc_printf("%s\\afl.input", out_dir);
